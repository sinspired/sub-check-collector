...
- **全量收集**: 移除 Code Search 和 Topics 的早期截断，先全量收集再统一排序
- **候选池验证**: 先按 `updated_at` 排序，只验证前 N*5 个候选（293 个中验证 250 个）
- **覆盖率提升**: 候选仓库从 111 增加到 297，覆盖率提升 160%

#### 并发处理

- **仓库并发**: 仓库处理改为 5 并发，大幅提升收集速度
- **移除延迟**: 移除仓库间固定 500ms 延迟

#### 去重优化

- **O(n) 去重**: 用 `baseUrlMap` 替代遍历，复杂度从 O(n²) 降到 O(n)
- **高效 base URL 匹配**: 同一文件不同格式只保留一个

### 🐛 Bug 修复

- **GitHub Token**: 修复 Windows CRLF 换行符导致的 401 错误
- **配置项错误**: 修复文件过滤误用仓库级 `maxDaysSinceUpdate` 的问题
- **429 重试**: HTTP 429 响应现在会实际重试而非直接失败
- **分支名解析**: 修复分支名含斜杠时 GitHub URL 解析截断的问题
- **Promise 容错**: `Promise.all` 改为 `Promise.allSettled`，单个策略失败不影响其他
- **注释误匹配**: 修复 YAML 注释中的 `sub-urls:` 被误匹配的问题
- **URL 过滤**: 修复目录链接、LICENSE 文件被误收集的问题
- **备份排序**: 备份文件改为按修改时间排序

### ✨ 功能增强

- **协议分类**: 新增 Hysteria、TUIC、WireGuard 协议识别
- **URL 路径推断**: 从 URL 路径推断订阅类型
- **Token 校验**: 启动时验证 GitHub Token 有效性，无效时跳过关键词搜索并警告
- **非订阅过滤**: 增强过滤规则（solidity/ethereum/evm/vpn-client 等）
...
- `.gitignore` 添加 `.mimocode/` 目录
- `.env.example` 与 `.env` 结构同步
- 文档全面更新
